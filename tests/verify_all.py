"""Full module verification with real Kimi LLM (skips heavy cognify)."""
import sys
import os
...
async def run_all():
    print("=" * 60)
    print("Research Cognitive Graph - Full Verification")
    print("=" * 60)
    ...
    # M1.2 Ingestion (without cognify)
    print("\n[M1.2] Ingestion Setup")
    try:
        from src.pipeline.ingest import ingest_paper
        report("ingest_imports", True)
        from src.parsing.grobid_client import GrobidClient
        import hashlib
        client = GrobidClient()
        paper = client.parse_pdf(TEST_PAPER)
        ...
        report("text_build", bool(paper.title))
    except Exception as e:
    ...
    # M2.3 Real LLM Extraction
    print("\n[M2.3] Academic Extraction Pipeline (Real LLM)")
    try:
        from src.parsing.grobid_client import GrobidClient
        ...
        report("methods_>=1", len(result["methods"]) >= 1, f"got={result['methods']}")
        report("problems_>=1", len(result["problems"]) >= 1, f"got={result['problems']}")
        report("insights_>=1", result["insights"] >= 1, f"count={result['insights']}")
        report("relations_>=1", result["relations"] >= 1, f"count={result['relations']}")
        print(f"       Methods: {result['methods']}")
        print(f"       Problems: {result['problems']}")
        print(f"       Insights: {result['insights']}")
    except Exception as e:
        report("extraction_llm", False, str(e))
        traceback.print_exc()
